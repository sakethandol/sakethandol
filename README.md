# 👋 Hey there! I'm **Saketh**

**Frontend Developer | Data Science Student | Python Full-Stack Learner | Big Data Enthusiast**

Crafting creative UIs, solving real-world problems, and building impactful projects 🚀

---

## 🚀 About Me

```python
class Developer:
    def __init__(self):
        self.name = "Andol Saketh"
        self.role = "Frontend + Python Full Stack Developer"
        self.specialization = "Data Science"
        self.current_projects = [
            "Portfolio Website", 
            "Django File Converter",
            "Expence Tracker",
            "OTT Market Dashboard" 
        ]
        self.location = "India 🇮🇳"
        self.goal = "Be Happy"
    
    def say_hi(self):
        return "Thanks for stopping by! Let's build something amazing together! ✨"
```
- ⚡ Turning caffeine into code since 2022
- 💼 Currently working on GarageMitra - revolutionizing garage management 
- 🎨 I debug with console.log() and I'm not ashamed to admit it
- 🧠 Strong interest in Frontend, Backend, Data Science & Big Data
- 🔥 Learning new technologies through real projects
- 😄 Fun fact: *My Tailwind designs come out cleaner than my room!*

---

## 🛠️ Tech Stack

### **Languages**
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### **Frontend**
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![Responsive Design](https://img.shields.io/badge/Responsive-Design-FF6B6B?style=for-the-badge&logo=responsive&logoColor=white)

### **Backend**
![Django](https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white)
![REST API](https://img.shields.io/badge/REST-API-009688?style=for-the-badge&logo=fastapi&logoColor=white)

### **Databases**
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=for-the-badge&logo=sqlite&logoColor=white)

### **Tools & Platforms**
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Apache Spark](https://img.shields.io/badge/Apache_Spark-E25A1C?style=for-the-badge&logo=apachespark&logoColor=white)

---

## 💼 Current Focus

### 🔧 **Portfolio Website**
Building a stunning showcase of my skills using modern UI with Tailwind + JavaScript

### 🔧 **Django File Converter**
A multi-format converter with user accounts, history tracking, and real-time status updates
- PDF ⇄ Word
- Excel ⇄ Image
- Toast notifications & progress tracking

### 🔧 **Online Voting System**
Full-stack Python + Django application with secure authentication, real-time updates, and clean dashboard


---

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=radical&hide_border=true&count_private=true" alt="GitHub Stats" height="165">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=radical&hide_border=true" alt="Top Languages" height="165">
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_USERNAME&theme=radical&hide_border=true" alt="GitHub Streak" />
</div>

---

## 💭 Philosophy

> *"Learn by building. Improve by practicing. Grow every single day."*

```python
class Mindset:
    beliefs = {
        "continuous_learning": True,
        "clean_code": "always",
        "building_projects": "love_it",
        "documentation": "not_my_favorite_but_necessary"
    }
    
    def daily_routine(self):
        return [
            "Write better code than yesterday 📝",
            "Learn something new 🧠",
            "Push to GitHub 🚀",
            "Stay consistent 💪"
        ]
```

---

## 🌱 Continuous Growth

- 🔹 Learning Full Stack Development with Django & React
- 🔹 Exploring Big Data technologies (Spark, Hadoop)
- 🔹 Mastering DSA for competitive coding
- 🔹 Sharpening frontend creativity with modern frameworks
- 🔹 Building real-world projects that solve problems

---

## 🤝 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/andol-saketh)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:sakethandol1817@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://andolsaketh.me)

</div>

---

<div align="center">

### ⭐ Thanks for visiting!

**P.S.** — *My Tailwind UI is fire 🔥, but my CSS without Tailwind… let's not talk about it 😅*
         - *My code works perfectly... until I explain it to someone, then the bugs magically appear 🪲✨*

![Visitor Count](https://profile-counter.glitch.me/YOUR_USERNAME/count.svg)

</div>
